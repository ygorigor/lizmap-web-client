...
var lizMap = function() {
 ...
 function updateMapSize(){
  ...
  function updateMiniDockSize() {
      ...
      mdmcv.css({ 'max-height': window.innerHeight - 100 + 'px' }).css('overflow-x', 'hidden').css('overflow-y', 'auto');
  }
  ...
  function addDock( dname, dlabel, dtype, dcontent, dicon){
      ...
      if( dtype == 'minidock'){
          docktab+='<div class="mini-dock-close" title="'+lizDict['generic.btn.close.title']+'" style="padding:7px;float:right;cursor:pointer;"><i class="icon-remove icon-white"></i></div>';
